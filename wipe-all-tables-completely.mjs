...
async function runCompleteWipeAndSetup() {
    console.log("🚀 Starting database wipe and seeding via Supabase API...");

    // 1. Wipe payroll and documents
    console.log("Wiping payroll_records...");
    try {
        const { error: err1 } = await supabaseAdmin.from('payroll_records').delete().neq('id', -1);
        if (err1) console.error("Warning/Error wiping payroll_records:", err1.message);
    } catch (e) {
        console.error("Exception wiping payroll_records:", e);
    }

    console.log("Wiping documents_archive...");
    try {
        const { error: err2 } = await supabaseAdmin.from('documents_archive').delete().neq('id', -1);
        if (err2) console.error("Warning/Error wiping documents_archive:", err2.message);
    } catch (e) {
        console.error("Exception wiping documents_archive:", e);
    }

    // 2. Wipe operational tables via DELETE
    console.log("Wiping report_services...");
    await supabaseAdmin.from('report_services').delete().neq('id', '00000000-0000-0000-0000-000000000000');

    console.log("Wiping used_parts...");
    await supabaseAdmin.from('used_parts').delete().neq('id', '00000000-0000-0000-0000-000000000000');

    console.log("Wiping inspection_reports...");
    await supabaseAdmin.from('inspection_reports').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    ...
    const { error: errEmployees } = await supabaseAdmin.from('employees').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    if (errEmployees) {
        console.error("❌ Failed to wipe employees:", errEmployees.message);
    } else {
        console.log("✅ Wiped all employees successfully!");
    }
    ...
    console.log("\n⭐️ DATABASE WIPE AND SEEDING COMPLETED SUCCESSFULLY! ⭐️");
}

runCompleteWipeAndSetup();
